<?php
// pages/modules/sms/employee_managment.php -> Technical Staff & Field Personnel HR Registry (SMS Directorate)
...
// Fetch Staff & Officers for SMS Unit
$staff_list = [];
$stmt = $mysqli->prepare("
    SELECT u.*, d.name as district_name 
    FROM users u 
    LEFT JOIN districts d ON u.district_id = d.id 
    WHERE (u.role = 'sms' OR u.id = ? OR (u.role = 'employee' AND (u.designation LIKE '%SMS%' OR u.service_category LIKE '%Technical%' OR u.service_category LIKE '%Epidemiology%'))) AND u.is_active = 1 
    ORDER BY u.id DESC
");
if ($stmt) {
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h3 class="fw-bold text-dark mb-1">
            <i class="bi bi-people-fill me-2" style="color: #820100;"></i>Technical Staff &amp; Field Personnel Management
        </h3>
        <p class="text-muted small mb-0">SMS Unit technical assistants, field vaccinators, laboratory technicians &amp; surveillance personnel</p>
    </div>
    <div class="d-flex gap-2">
        <button class="btn text-light shadow-sm fw-bold" style="background-color: #820100;" data-bs-toggle="modal" data-bs-target="#addEmployeeModal">
            <i class="bi bi-person-plus-fill me-2"></i>Register Technical Officer
        </button>
            ...
            <table id="employeeTable" class="table table-hover align-middle w-100">
                ...
                <tbody>
                    <?php foreach ($staff_list as $emp): ?>
                        <tr>
                            <td><span class="fw-bold text-primary"><?= htmlspecialchars($emp['service_number'] ?: ($emp['emp_id'] ?: 'SMS-EMP-' . $emp['id'])) ?></span></td>
                            <td>
                            ...
                            <td><span class="badge bg-light text-dark border"><?= htmlspecialchars($emp['designation'] ?: 'SMS Technical Staff') ?></span></td>
                            <td><span class="badge bg-info-subtle text-info border border-info px-2"><?= ucwords(str_replace('_', ' ', $emp['role'] ?? 'sms')) ?></span></td>
                            <td class="small"><?= htmlspecialchars($emp['service_category'] ?: 'Technical Field Services') ?></td>
                            ...
                            <td class="text-center text-nowrap">
                                ...
                                <button class="btn btn-sm btn-outline-primary me-1 btn-edit-emp"
                                    ...
                                    data-designation="<?= htmlspecialchars($emp['designation'] ?? '') ?>"
                                    data-user_role="<?= htmlspecialchars($emp['role'] ?? 'sms') ?>"
                                    data-service_category="<?= htmlspecialchars($emp['service_category'] ?? '') ?>"
                                    data-email="<?= htmlspecialchars($emp['email'] ?? '') ?>"
...
document.addEventListener('DOMContentLoaded', function() {
    $(document).on('click', '.btn-view-emp', function() {
        ...
        $('#view_designation').text(btn.data('designation') || 'Technical Staff');
        $('#view_role').text(btn.data('role') || 'SMS');
