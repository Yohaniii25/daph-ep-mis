                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="batch_no" id="edit_batch_no" class="form-select form-select-sm fw-bold text-dark border-secondary">
                                <option value="" selected>-- Select Batch or None --</option>
                                <?php
                                $batch_opts = $mysqli->query("SELECT batch_number, expiry_date FROM vaccine_batches ORDER BY id DESC");
                                if ($batch_opts) {
                                    while ($opt = $batch_opts->fetch_assoc()):
                                        $b_exp = !empty($opt['expiry_date']) ? date('Y-m-d', strtotime($opt['expiry_date'])) : '';
                                ?>
                                    <option value="<?= htmlspecialchars($opt['batch_number'], ENT_QUOTES) ?>" data-expiry="<?= $b_exp ?>">
                                        <?= htmlspecialchars($opt['batch_number']) ?><?= $b_exp ? ' (Exp: ' . $b_exp . ')' : '' ?>
                                    </option>
                                <?php 
                                    endwhile;
                                }
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" id="edit_opening_balance" name="opening_balance" class="form-control form-control-sm border-secondary fw-bold text-primary" min="0" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label small fw-bold">Received Doses</label>
                            <input type="number" id="edit_received_doses" name="received_doses" class="form-control form-control-sm border-secondary fw-bold text-success" min="0" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label small fw-bold">Used Doses</label>
                            <input type="number" id="edit_used_doses" name="used_doses" class="form-control form-control-sm border-secondary fw-bold text-info" min="0" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label small fw-bold">Spoilt / Damaged Doses</label>
                            <input type="number" id="edit_spoilt_doses" name="spoilt_damaged_doses" class="form-control form-control-sm border-secondary fw-bold text-danger" min="0" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label small fw-bold">Transferred Doses</label>
                            <input type="number" id="edit_transferred_doses" name="transferred_doses" class="form-control form-control-sm border-secondary fw-bold" min="0" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label small fw-bold">Closing Balance (Doses)</label>
                            <input type="number" id="edit_closing_balance" name="closing_balance" class="form-control form-control-sm border-secondary fw-bold bg-light text-dark" min="0" required readonly>
                        </div>
                        
                        <div class="col-md-6">
                            <label class="form-label small fw-bold">Expiry Date</label>
                            <input type="date" name="expiry_date" id="edit_expiry_date" class="form-control form-control-sm border-secondary">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold">Remarks</label>
                            <input type="text" name="remarks" id="edit_remarks" class="form-control form-control-sm border-secondary">
                        </div>
                    </div>
                </div>
                <div class="modal-footer py-2 bg-light border-0">
                    <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-sm text-light fw-bold px-4" style="background-color:#820100;"><i class="bi bi-save me-1"></i>Update Record</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const batchSelect = document.getElementById('edit_batch_no');
    const expiryInput = document.getElementById('edit_expiry_date');
    const vaxSelect = document.getElementById('edit_vaccine_name');
    ...
    if (vaxSelect) {
        vaxSelect.addEventListener('change', function() {
            ...
            if (selectedExpiry && selectedExpiry !== 'N/A' && !expiryInput.value) {
                expiryInput.value = selectedExpiry;
            }
        });
    }
});
