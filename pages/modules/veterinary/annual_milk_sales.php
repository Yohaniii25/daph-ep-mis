...
header("Location: milk_collection_details.php?tab=sales" . $qs);
exit();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'add') {
            ...
            $sales_center_name = trim($_POST['sales_center_name']);
            $address = trim($_POST['address']);
            $contact_no = trim($_POST['contact_no']);
            $fresh_milk_lit_per_month = floatval($_POST['fresh_milk_lit_per_month']);
            ...
            $insert_query = "
                ...
                (vs_range, sales_center_name, address, contact_no, 
                 fresh_milk_lit_per_month, yoghurt_lit_per_month, curd_lit_per_month, 
                 ice_cream_lit_per_month, ghee_lit_per_month, other_milk_product_lit_per_month, 
                 total_lit_per_month, income_rs_per_month)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ";
            $stmt = $mysqli->prepare($insert_query);
            if ($stmt) {
                // vs_range (s), sales_center_name (s), address (s), contact_no (s),
                // fresh_milk_lit_per_month (d), yoghurt_lit_per_month (d), curd_lit_per_month (d), 
                // ice_cream_lit_per_month (d), ghee_lit_per_month (d), other_milk_product_lit_per_month (d), 
                // total_lit_per_month (d), income_rs_per_month (d) -> ssssdddddddd (12 placeholders)
                $stmt->bind_param(
                    "ssssdddddddd",
                    $vs_range,
                    $sales_center_name,
                ...
                if ($stmt->execute()) {
                    header("Location: annual_milk_sales.php?status=success&msg=" . urlencode("Sales outlet added successfully."));
                } else {
                    header("Location: annual_milk_sales.php?status=error&msg=" . urlencode("Failed to write to database: " . $stmt->error));
                }
                $stmt->close();
            } else {
                header("Location: annual_milk_sales.php?status=error&msg=" . urlencode("Query preparation failed: " . $mysqli->error));
            }
            exit();
            ...
            $update_query = "
                ...
                SET vs_range = ?, sales_center_name = ?, address = ?, contact_no = ?, 
                    fresh_milk_lit_per_month = ?, yoghurt_lit_per_month = ?, curd_lit_per_month = ?, 
                    ice_cream_lit_per_month = ?, ghee_lit_per_month = ?, other_milk_product_lit_per_month = ?, 
                    total_lit_per_month = ?, income_rs_per_month = ?
            ...
            if ($stmt) {
                ...
                // fresh_milk_lit_per_month (d), yoghurt_lit_per_month (d), curd_lit_per_month (d), 
                // ice_cream_lit_per_month (d), ghee_lit_per_month (d), other_milk_product_lit_per_month (d), 
                // total_lit_per_month (d), income_rs_per_month (d), id (i), vs_range (s) -> ssssddddddddis (14 placeholders)
                $stmt->bind_param(
                    "ssssddddddddis",
                    $vs_range,
                    $sales_center_name,
                ...
                if ($stmt->execute()) {
                    header("Location: annual_milk_sales.php?status=success&msg=" . urlencode("Sales outlet updated successfully."));
                } else {
                    header("Location: annual_milk_sales.php?status=error&msg=" . urlencode("Failed to update database: " . $stmt->error));
                }
                $stmt->close();
            } else {
                header("Location: annual_milk_sales.php?status=error&msg=" . urlencode("Query preparation failed: " . $mysqli->error));
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $mysqli->prepare("DELETE FROM milk_product_sales_centers WHERE id = ? AND vs_range = ?");
    if ($stmt) {
        $stmt->bind_param("is", $id, $range_name);
        if ($stmt->execute()) {
            header("Location: annual_milk_sales.php?status=success&msg=" . urlencode("Record deleted successfully."));
        } else {
            header("Location: annual_milk_sales.php?status=error&msg=" . urlencode("Failed to delete record."));
        }
        $stmt->close();
    }
    exit();
...
$records = [];
if (!empty($range_name)) {
    $stmt = $mysqli->prepare("SELECT * FROM milk_product_sales_centers WHERE vs_range = ? ORDER BY id DESC");
    if ($stmt) {
...
foreach ($records as $r) {
    $summary['total_sales'] += floatval($r['total_lit_per_month']);
...
<link rel="stylesheet" href="../../../assets/css/veterinary.css">
        ...
        <div class="mb-4 d-flex justify-content-between align-items-center">
            <div>
                <h2 class="h4 fw-bold mb-1" style="color: #370709;">Details of Milk Based Product Sales Centers</h2>
                <p class="text-muted small mb-0">Record and track sales volumes and products for <strong class="text-dark"><?= htmlspecialchars($range_name) ?></strong> (<?= htmlspecialchars($district_name) ?> District)</p>
            </div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-6 col-lg-6">
                <div class="card shadow-sm border-0 border-start border-success border-4 text-center">
                    <div class="card-body py-3">
                        <span class="text-muted small text-uppercase fw-bold">Total Sales Centers</span>
            ...
            <div class="col-6 col-lg-6">
                <div class="card shadow-sm border-0 border-start border-primary border-4 text-center">
                    <div class="card-body py-3">
                        <span class="text-muted small text-uppercase fw-bold">Total Sales (Litre/Month)</span>
                        <h4 class="mb-0 fw-bold text-primary mt-1"><?= number_format($summary['total_sales'], 2) ?> Litres</h4>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                <button class="btn btn-primary w-100 py-3 text-light border-0 shadow-sm d-flex flex-column align-items-center justify-content-center" style="background-color: #820100; min-height: 105px;" data-bs-toggle="modal" data-bs-target="#addCenterModal">
                                    ...
                                    <span class="small fw-bold text-uppercase">Add Sales Centre</span>
                                </button>
                            </div>
                            <div class="col-md-3">
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center border-bottom">
                <h5 class="card-title mb-0 fw-bold text-dark"><i class="bi bi-table me-2"></i>Milk Based Product Sales Centers log</h5>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0 table-bordered" id="centerTable" style="min-width: 1700px;">
                        <thead class="table-light text-secondary small uppercase">
                            <tr>
                                ...
                                <th rowspan="2" class="align-middle">Sales Center Name</th>
                                <th rowspan="2" class="align-middle">Address</th>
                                <th rowspan="2" class="align-middle">Contact No</th>
                                <th colspan="7" class="text-center">Milk Process lit / month</th>
                            ...
                            <tr class="table-light text-secondary small">
                                <th class="text-end">Fresh Milk lit/Month</th>
                        ...
                        <tbody class="small">
                            <?php foreach ($records as $row): ?>
                                <tr
                                    ...
                                    data-center_name="<?= htmlspecialchars($row['sales_center_name']) ?>"
                                    data-address="<?= htmlspecialchars($row['address']) ?>"
                                    data-contact_no="<?= htmlspecialchars($row['contact_no']) ?>"
                                    data-fresh_milk="<?= htmlspecialchars($row['fresh_milk_lit_per_month']) ?>"
                                    ...
                                    <td class="fw-bold"><?= htmlspecialchars($row['sales_center_name']) ?></td>
                                    <td><?= nl2br(htmlspecialchars($row['address'])) ?></td>
                                    <td><?= htmlspecialchars($row['contact_no']) ?></td>
                                    <td class="text-end font-monospace"><?= number_format($row['fresh_milk_lit_per_month'], 2) ?></td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="annual_milk_sales.php?action=delete&id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-danger btn-delete" title="Delete"><i class="bi bi-trash-fill"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
        ...
        <form method="POST" id="addCenterForm">
            ...
            <div class="modal-content">
                <div class="modal-header" style="background-color: #370709; color: white;">
                    <h5 class="modal-title" id="addCenterModalLabel"><i class="bi bi-plus-circle me-2"></i>Add Milk Sales Centre</h5>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-bold">Sales Center Name</label>
                            <input type="text" name="sales_center_name" class="form-control" placeholder="e.g. Balapitiya Outlet" required>
                        </div>
                        <div class="col-md-12">
                        ...
                        <div class="col-12 mt-4">
                            <h6 class="fw-bold border-bottom pb-2 text-secondary">Milk Process Volume (lit / month)</h6>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Fresh Milk lit/Month</label>
                            <input type="number" step="0.01" name="fresh_milk_lit_per_month" id="add_fresh_milk" class="form-control volume-input" value="0.00">
                        </div>
                        <div class="col-md-4">
...
<div class="modal fade" id="editCenterModal" tabindex="-1" aria-labelledby="editCenterModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form method="POST" id="editCenterForm">
            ...
            <div class="modal-content">
                <div class="modal-header" style="background-color: #370709; color: white;">
                    <h5 class="modal-title" id="editCenterModalLabel"><i class="bi bi-pencil-fill me-2"></i>Edit Milk Sales Centre</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-bold">Sales Center Name</label>
                            <input type="text" name="sales_center_name" id="edit_center_name" class="form-control" required>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label fw-bold">Address</label>
                        ...
                        <div class="col-md-4">
                            <label class="form-label">Fresh Milk lit/Month</label>
                            <input type="number" step="0.01" name="fresh_milk_lit_per_month" id="edit_fresh_milk" class="form-control edit-volume-input">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Yoghurt lit / month</label>
...
<div class="modal fade" id="viewCenterModal" tabindex="-1" aria-labelledby="viewCenterModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #370709; color: white;">
                <h5 class="modal-title" id="viewCenterModalLabel"><i class="bi bi-eye-fill me-2"></i>Milk Sales Center Details</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <table class="table table-bordered table-striped">
                    <tbody>
                        ...
                        <tr>
                            <th>Fresh Milk lit/Month</th>
                            <td id="view_fresh_milk" class="font-monospace"></td>
                        </tr>
                        <tr>
...
$(document).ready(function() {
    ...
    function calculateAddTotal() {
        var fresh = parseFloat($("#add_fresh_milk").val()) || 0;
        ...
        var total = fresh + yoghurt + curd + ice_cream + ghee + other;
        $("#add_total").val(total.toFixed(2));
    }
    
    function calculateEditTotal() {
        var fresh = parseFloat($("#edit_fresh_milk").val()) || 0;
    ...
    $(document).on(\'click\', \'.btn-view\', function() {
        ...
        var fresh = parseFloat($row.data(\'fresh_milk\')) || 0;
        ...
        $(\'#view_fresh_milk\').text(fresh.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}));
    ...
    $(document).on(\'click\', \'.btn-edit\', function() {
        ...
        $(\'#edit_fresh_milk\').val($row.data(\'fresh_milk\'));
