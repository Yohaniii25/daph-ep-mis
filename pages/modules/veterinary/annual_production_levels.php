    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'add') {
            ...
            if ($check_result->num_rows > 0) {
                header("Location: annual_production_levels.php?year=$selected_year&status=error&msg=" . urlencode("A record for the year $year already exists."));
                exit();
            }
            $check_stmt->close();
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    header("Location: annual_production_levels.php?year=$year&status=success&msg=" . urlencode("Production levels added successfully."));
                } else {
                    header("Location: annual_production_levels.php?year=$selected_year&status=error&msg=" . urlencode("Failed to write to database: " . $stmt->error));
                }
                $stmt->close();
            } else {
                header("Location: annual_production_levels.php?year=$selected_year&status=error&msg=" . urlencode("Query preparation failed."));
            }
            exit();
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    header("Location: annual_production_levels.php?year=$year&status=success&msg=" . urlencode("Production levels updated successfully."));
                } else {
                    header("Location: annual_production_levels.php?year=$selected_year&status=error&msg=" . urlencode("Failed to update database: " . $stmt->error));
                }
                $stmt->close();
            } else {
                header("Location: annual_production_levels.php?year=$selected_year&status=error&msg=" . urlencode("Query preparation failed."));
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            header("Location: annual_production_levels.php?year=$selected_year&status=success&msg=" . urlencode("Record deleted successfully."));
        } else {
            header("Location: annual_production_levels.php?year=$selected_year&status=error&msg=" . urlencode("Failed to delete record."));
        }
        $stmt->close();
    }
    exit();
...
$records = [];
if (!empty($range_id)) {
    $stmt = $mysqli->prepare("SELECT * FROM annual_production_levels WHERE range_id = ? AND report_year = ? ORDER BY id DESC");
    if ($stmt) {
    ...
    <main class="container-fluid px-4 pt-4">
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center border-bottom">
                <h5 class="card-title mb-0 fw-bold text-dark"><i class="bi bi-table me-2"></i>Production Levels Log - <?= $selected_year ?></h5>
            </div>
                                ...
                                <?php foreach ($records as $row): ?>
                                    <tr 
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="annual_production_levels.php?year=<?= $selected_year ?>&action=delete&id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-danger btn-delete" title="Delete"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
