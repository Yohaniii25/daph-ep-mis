...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$user_id || empty($counterfoil_type) || !$available_quantity) {
        echo json_encode(['success' => false, 'message' => 'Validation error: All key indicators must be specified.']);
        exit();
    }

    $stmt = $mysqli->prepare("INSERT INTO counterfoil_assets (user_id, district_id, range_id, counterfoil_type, current_condition, available_quantity, purchase_date, remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    if ($stmt) {
        $stmt->bind_param("iiisssis", $user_id, $district_id, $range_id, $counterfoil_type, $current_condition, $available_quantity, $purchase_date, $remarks);
        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Counterfoil registration successfully.']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Transactional writing fault: ' . $stmt->error]);
        }
        $stmt->close();
    }
}