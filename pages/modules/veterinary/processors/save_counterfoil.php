...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("INSERT INTO counterfoil_assets (user_id, district_id, range_id, counterfoil_type, current_condition, available_quantity, purchase_date, remarks, unit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if ($stmt) {
        $stmt->bind_param("iiisssiss", $user_id, $district_id, $range_id, $counterfoil_type, $current_condition, $available_quantity, $purchase_date, $remarks, $unit);
        if ($stmt->execute()) {
