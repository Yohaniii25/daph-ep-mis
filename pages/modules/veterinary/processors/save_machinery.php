...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("INSERT INTO machinery_assets (user_id, district_id, range_id, machinery_type, current_condition, available_quantity, initial_count, received_quantity, purchase_date, remarks, unit, issue_order_no, received_from, receipt_no, specification, removal_status, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', 1)");
    if ($stmt) {
        $stmt->bind_param("iiisssiisssssss", $user_id, $district_id, $range_id, $machinery_type, $current_condition, $available_quantity, $initial_count, $received_quantity, $purchase_date, $remarks, $unit, $issue_order_no, $received_from, $receipt_no, $specification);
        if ($stmt->execute()) {
