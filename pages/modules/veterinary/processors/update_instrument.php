...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id || empty($instrument_type) || $available_quantity === false || $available_quantity < 0) {
    ...
    $stmt = $mysqli->prepare("UPDATE instrument_assets SET instrument_type = ?, current_condition = ?, available_quantity = ?, initial_count = ?, purchase_date = ?, remarks = ?, unit = ? WHERE id = ?");
    if ($stmt) {
        $stmt->bind_param("ssiisssi", $instrument_type, $current_condition, $available_quantity, $initial_count, $purchase_date, $remarks, $unit, $id);
        if ($stmt->execute()) {
