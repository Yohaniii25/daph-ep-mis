...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("UPDATE instrument_assets SET instrument_type = ?, current_condition = ?, available_quantity = ?, purchase_date = ?, remarks = ? WHERE id = ?");
    if ($stmt) {
        $stmt->bind_param("ssissi", $instrument_type, $current_condition, $available_quantity, $purchase_date, $remarks, $id);
        if ($stmt->execute()) {
