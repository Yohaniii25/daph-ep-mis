...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("UPDATE instrument_assets SET instrument_type = ?, current_condition = ?, available_quantity = ?, initial_count = ?, received_quantity = ?, purchase_date = ?, remarks = ?, unit = ?, issue_order_no = ?, received_from = ?, receipt_no = ?, specification = ? WHERE id = ?");
    if ($stmt) {
        $stmt->bind_param("ssiiisssssssi", $instrument_type, $current_condition, $available_quantity, $initial_count, $received_quantity, $purchase_date, $remarks, $unit, $issue_order_no, $received_from, $receipt_no, $specification, $id);
        if ($stmt->execute()) {
