...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($report_year) || empty($report_month)) {
        header("Location: ../ear_tag_balance.php?status=error&msg=Missing+required+fields");
        exit();
    }

    if ($action === 'create') {
        if (empty($district_id) || empty($range_id)) {
            header("Location: ../ear_tag_balance.php?status=error&msg=Invalid+surgeon+range+context");
            exit();
        }

        $stmt = $mysqli->prepare("INSERT INTO `ear_tag_usage` 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt) {
            header("Location: ../ear_tag_balance.php?status=error&msg=" . urlencode($mysqli->error));
            exit();
        }
        ...
        if ($stmt->execute()) {
            header("Location: ../ear_tag_balance.php?status=success&msg=Record+Created+Successfully");
            exit();
        } else {
            header("Location: ../ear_tag_balance.php?status=error&msg=" . urlencode($stmt->error));
            exit();
        }

    } elseif ($action === 'update' && $id > 0) {
        $stmt = $mysqli->prepare("UPDATE `ear_tag_usage` SET 
            report_year = ?, 
            report_month = ?, 
            opening_balance = ?, 
            received_qty = ?, 
        ...
        if ($stmt->execute()) {
            header("Location: ../ear_tag_balance.php?status=success&msg=Record+Updated+Successfully");
            exit();
        } else {
            header("Location: ../ear_tag_balance.php?status=error&msg=" . urlencode($stmt->error));
            exit();
    ...
    if ($id <= 0) {
        header("Location: ../ear_tag_balance.php?status=error&msg=Invalid+Record+ID");
        exit();
    }

    $stmt = $mysqli->prepare("DELETE FROM `ear_tag_usage` WHERE id = ? AND range_id = ?");
    if (!$stmt) {
        header("Location: ../ear_tag_balance.php?status=error&msg=" . urlencode($mysqli->error));
        exit();
    }
    ...
    if ($stmt->execute()) {
        header("Location: ../ear_tag_balance.php?status=success&msg=Record+Deleted");
        exit();
    } else {
        header("Location: ../ear_tag_balance.php?status=error&msg=" . urlencode($stmt->error));
        exit();
