...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("UPDATE diary_tasks SET task_date = ?, place = ?, activity = ?, time_duration = ? WHERE id = ? AND user_id = ?");
    if ($stmt) {
        $stmt->bind_param("ssssii", $date, $place, $task, $time_duration, $id, $user_id);
        if ($stmt->execute()) {
            header("Location: ../daily_diary.php?status=updated&year=" . $year);
        } else {
            header("Location: ../daily_diary.php?status=db_error&year=" . $year);
        }
        $stmt->close();
