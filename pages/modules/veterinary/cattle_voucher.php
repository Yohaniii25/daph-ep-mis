...
if (!empty($records)) {
    // Latest balance is closing balance of latest month recorded
...
<link rel="stylesheet" href="../../../assets/css/veterinary.css">
        ...
        <div class="mb-4 d-flex justify-content-between align-items-center">
            <div>
                <h2 class="h4 fw-bold mb-1" style="color: #370709;">Cattle Voucher - Monthly Returns</h2>
                <p class="text-muted small mb-0">Manage monthly cattle voucher stock levels for <strong class="text-dark"><?= htmlspecialchars($range_name) ?></strong> (<?= htmlspecialchars($district_name) ?> District)</p>
            </div>
            ...
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body pt-0">
                        <div class="row g-3">
                            <div class="col-md-3">
                                <button class="btn w-100 py-3 text-light border-0 shadow-sm d-flex flex-column align-items-center justify-content-center" style="background-color: #820100; min-height: 105px;" data-bs-toggle="modal" data-bs-target="#addCattleVoucherModal">
                                    <i class="bi bi-plus-circle fs-3 mb-1"></i>
                                    <span class="small fw-bold text-uppercase">Log Voucher Returns</span>
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center border-bottom">
                <h5 class="card-title mb-0 fw-bold text-dark"><i class="bi bi-table me-2"></i>Cattle Voucher Returns Log - <?= $selected_year ?></h5>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0" id="cattleVoucherTable" style="min-width: 900px;">
                                ...
                                <?php foreach ($records as $row): ?>
                                    <tr 
                                        ...
                                        <td class="text-center">
                                            <button class="btn btn-sm btn-outline-primary btn-edit-voucher" title="Edit"><i class="bi bi-pencil-square"></i></button>
                                            <a href="processors/cattle_voucher_crud.php?action=delete&id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-danger btn-delete-voucher" title="Delete"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
...
$(document).ready(function() {
    ...
    if ($.fn.DataTable.isDataTable("#cattleVoucherTable")) {
        $("#cattleVoucherTable").DataTable().destroy();
    }
    $("#cattleVoucherTable").DataTable({
        "order": [[0, "desc"]],
        "pageLength": 12,
    ...
    $(document).on(\'input\', \'.voucher-calc-trigger\', function() {
        const modal = $(this).closest(\'.modal\');
        const isEdit = modal.attr(\'id\') === \'editCattleVoucherModal\';
    ...
    $(document).on(\'click\', \'.btn-edit-voucher\', function() {
        ...
        $(\'.voucher-calc-trigger\').first().trigger(\'input\');

        new bootstrap.Modal(document.getElementById(\'editCattleVoucherModal\')).show();
    });
    ...
    $(document).on(\'click\', \'.btn-delete-voucher\', function(e) {
        ...
        Swal.fire({
            icon: \'warning\',
            title: \'Delete Voucher Return Records?\',
            html: \'Are you sure you want to permanently delete the cattle voucher return records for <strong>\' + monthName + \'</strong>?<br>This action cannot be undone.\',
