...
// Fetch Cash Book Summary records matching year filter and range
$records = [];
if (!empty($range_id)) {
    $records_sql = "
        SELECT id, report_year, report_month, item_name, quantity_sold, unit_price, total_amount, amount_deposited 
...
<link rel="stylesheet" href="../../../assets/css/veterinary.css">
        ...
        <div class="mb-4 d-flex justify-content-between align-items-center">
            <div>
                <h2 class="h4 fw-bold mb-1" style="color: #370709;">Cash Book Summary</h2>
                <p class="text-muted small mb-0">Record and track item sales and deposits for <strong class="text-dark"><?= htmlspecialchars($range_name) ?></strong> (<?= htmlspecialchars($district_name) ?> District)</p>
            </div>
            ...
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body pt-0">
                        <div class="row g-3">
                            <div class="col-md-3">
                                <button class="btn w-100 py-3 text-light border-0 shadow-sm d-flex flex-column align-items-center justify-content-center" style="background-color: #820100; min-height: 105px;" data-bs-toggle="modal" data-bs-target="#addCashBookSummaryModal">
                                    <i class="bi bi-plus-circle fs-3 mb-1"></i>
                                    <span class="small fw-bold text-uppercase">Add Cash Book Record</span>
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3">
                <h6 class="mb-0 fw-bold"><i class="bi bi-list-task me-2 text-primary"></i>Cash Book Summary Records for <?= htmlspecialchars($selected_year) ?></h6>
            </div>
            <div class="card-body p-3">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0" id="cashBookTable">
                        <thead class="bg-light small text-uppercase">
                            <tr>
                                ...
                                <th class="text-end">Amount Deposited (LKR)</th>
                        ...
                        <tbody>
                            <?php if (empty($records)): ?>
                                <tr>
                                    <td colspan="7" class="text-center text-muted py-4">No cash book summary records found for year <?= htmlspecialchars($selected_year) ?>.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($records as $row): ?>
                                    <tr
                                        ...
                                        data-qty="<?= htmlspecialchars($row['quantity_sold']) ?>"
                                        data-price="<?= htmlspecialchars($row['unit_price']) ?>"
                                        data-total="<?= htmlspecialchars($row['total_amount']) ?>"
                                        data-deposited="<?= htmlspecialchars($row['amount_deposited']) ?>">
                                        ...
                                        <td class="text-end fw-bold text-dark"><?= number_format($row['total_amount'], 2) ?></td>
                                        <td class="text-end fw-bold text-success"><?= number_format($row['amount_deposited'], 2) ?></td>
                                        <td class="text-end">
                                            <button class="btn btn-sm btn-outline-primary btn-edit-cb" title="Edit"><i class="bi bi-pencil-square"></i></button>
                                            <button class="btn btn-sm btn-outline-danger btn-delete-cb" title="Delete"><i class="bi bi-trash"></i></button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
...
<!-- Add Modal -->
<?php include 'models/add_cash_book_summary_modal.php'; ?>

<!-- Edit Modal -->
<?php include 'models/edit_cash_book_summary_modal.php'; ?>
...
$(document).ready(function() {
    ...
    if (status === 'success') {
        Swal.fire({
            icon: 'success',
            title: 'Record Saved!',
            text: 'Cash Book entry was processed successfully.',
    ...
    $(document).on('click', '.btn-edit-cb', function() {
        ...
        $('#edit_qty_sold').val($row.data('qty'));
        $('#edit_unit_price').val($row.data('price'));
        $('#edit_total_amount').val($row.data('total'));
        $('#edit_amount_deposited').val($row.data('deposited'));

        new bootstrap.Modal(document.getElementById('editCashBookSummaryModal')).show();
    });

    // AJAX Delete Confirmation Click Handler
    $(document).on('click', '.btn-delete-cb', function() {
        var $row = $(this).closest('tr');
        var recordId = $row.data('id');
        var name = $row.data('item') || 'this record';

        Swal.fire({
            icon: 'warning',
            title: 'Delete Cash Book Record?',
